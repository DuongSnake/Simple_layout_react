    ...
    try {
      ...
      if (response.type.endsWith('/fulfilled')) {
        ...
        if (response.payload.data != null && response.payload.data.token) {
          sessionStorage.setItem(ACCESS_TOKEN_USER, response.payload.data.token);
          // sessionStorage.setItem(PAGE_LOGIN, "user");
          sessionStorage.setItem(USER_NAME_USER, response.payload.data.username);
        }
        // Redirect to assignment-register
        const redirectUrl = location.state?.urlAfterLoginSuccess || "/user/assignment-register";
        if("/user/login" == location.state?.urlAfterLoginSuccess ||"/user/login" == location.state?.urlAfterLoginSuccess){
        navigate("/user/assignment-register");
        }else{
    ...
    <div class="w-full max-w-xl p-6 space-y-8 sm:p-8 bg-white rounded-lg shadow dark:bg-gray-800">
        ...
        <div class="mt-8 space-y-6" action="#">
            ...
            <div class="flex items-start">
                <a href="/user/forgot-password" class="ml-auto text-sm text-primary-700 hover:underline dark:text-primary-500">Đặt lại mật khẩu?</a>
            </div>
