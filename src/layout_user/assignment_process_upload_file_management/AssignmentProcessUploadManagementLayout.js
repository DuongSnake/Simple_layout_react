...
import { selectListAssignmentProcessApi, insertListFileAssignmentProcessApi, updateListFileAssignmentProcessApi, selectListFileAssignmentProcessApi } from "./AssignmentProcessUploadManagementAPI";
...
function AssignmentProcessUploadManagementLayout() {
  ...
  const handleSelectAllChange = (e) => {
    if (e.target.checked) {
      ...
      const allPeriodAssignmentIds = new Set(
        listDataAssignmentStudentRegisterLoading?.map((assignmentProcess, idx) => assignmentProcess?.assignmentStudentRegisterId ?? idx) || []
      );
  ...
  }, [selectedAssignmentProcess, userIdGetFromAccountLogin, listDataFileAssignmentRegister]);
  ...
  const handleFormSubmitEditAdmissionPeriod = (event) => {
    event.preventDefault();
    console.log('Submitting edit form with data:', formDataEdit.listFile);
  ...
  const handleUpdate = async () => {
    ...
    if (formDataEdit.fileUpload instanceof File) {
      formData123.append("listFile", formDataEdit.listFile);
    }
    ...
    // console.log("form data:"+JSON.stringify(formData123));
      const response = await dispatch(updateListFileAssignmentProcessApi(formData123));
  ...
  const handleFileChangeUpdate = (event) => {
    ...
    setFormDataEdit((prev) => ({ ...prev, listFile: file }));
  };
              ...
              <div className="p-6 space-y-6">
                <form>
                  <div className="grid grid-cols-6 gap-6">
                    ...
                    <div className="grid grid-cols-6 gap-6">
                        <div className="col-span-6 sm:col-span-3">
                        <label htmlFor="file_input_docs" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Tệp tài liệu docs</label>
                        ...
                        shadow-xs placeholder:text-body" id="file_input_docs" type="file" onChange={handleFileChangeUpdate} />
                      </div>
                        <div className="col-span-6 sm:col-span-3">
                        {/* {editFileName ? (
                          <p className="mt-2 text-sm text-gray-500 dark:text-gray-400" style={{marginBottom: '0px'}, {marginTop: '26px'}}>Tên tệp đã tải lên trước đó: {editFileName}</p>
                        ) : (
                          <p className="mt-2 text-sm text-gray-500 dark:text-gray-400" style={{marginBottom: '0px'}, {marginTop: '26px'}}>Chưa có tệp nào được tải lên trước đó.</p>
                        )} */}
                    ...
                    <div className="grid grid-cols-6 gap-6">
                        <div className="col-span-6 sm:col-span-3">
                        <label htmlFor="file_input_ppt" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Tệp tài liệu trình chiếu</label>
                        <input className="cursor-pointer bg-neutral-secondary-medium border border-default-medium 
                        text-heading text-sm rounded-base focus:ring-brand focus:border-brand block w-full 
                        shadow-xs placeholder:text-body" id="file_input_ppt" type="file" onChange={handleFileChangeUpdate} />
                      </div>
                        <div className="col-span-6 sm:col-span-3">
                        {/* {editFileName ? (
                          <p className="mt-2 text-sm text-gray-500 dark:text-gray-400" style={{marginBottom: '0px'}, {marginTop: '26px'}}>Tên tệp đã tải lên trước đó: {editFileName}</p>
