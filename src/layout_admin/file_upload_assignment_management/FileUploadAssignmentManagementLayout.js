...
import { selectListAssignmentByPeriodTimeApi, createApi, updateApi, deleteApi, selectListFileUploadApi, downloadFileAssignmentProcessApi } from "./FileUploadAssignmentManagementAPI";
...
import '../.././App.css';
function FileUploadAssignmentManagement() {
  ...
  const listDataAssignmentRegister = useSelector(state => state.fileUploadAssignmentManagement.selectListFileUpload.data);
  const totalRecord = useSelector(state => state.fileUploadAssignmentManagement.selectListFileUpload.totalRecord);
  const listDataAssignmentStudentRegisterLoading = useSelector(state => state.fileUploadAssignmentManagement.selectListFileUpload.loading);
  ...
  const handleSelectAllChange = (e) => {
    if (e.target.checked) {
      ...
      const allScoreAssignmentIds = new Set(
        listDataAssignmentRegister?.map((assignmentRegister, idx) => assignmentRegister?.fileId ?? idx) || []
      );
  ...
  const handlePeriodAssignmentCheckboxChange = (scoreId) => {
    setSelectedPeriodAssignment((prevSelected) => {
      ...
      console.log('Selected score IDs:', JSON.stringify(listDataAssignmentRegister));
      //Set value for edit form when click checkbox of scoreId
      listDataAssignmentRegister.forEach(assignmentRegister => {
        if (assignmentRegister.fileId === scoreId) {
          setFormDataEdit({
            fileId: assignmentRegister?.fileId || '',
            fileName: assignmentRegister?.fileName || '',
            fileType: assignmentRegister?.fileType || '',
            ...
            assignmentName: assignmentRegister?.assignmentName || '',
  ...
  const handleCreate = async () => {
    ...
    formData123.append(
      "assignmentRegisterId",
      formDataEdit.assignmentRegisterId || 0
    );
  ...
  const handleFileChangeAdd = (event) => {
    ...
    if (!file) {
      ...
      setFormData((prev) => ({ ...prev, fileUploadContent: '' }));
      return;
    }

    setSelectedFileAdd(file);
    setFormData((prev) => ({ ...prev, fileUploadContent: file }));
  };
    // Handle file selection
  const handleFileChangeUpdate = (event) => {
    ...
    setFormDataEdit((prev) => ({ ...prev, fileUploadContent: file }));
  };
  ...
  const handleSelectListScoreAssignments = async (pageNum, pageSize) => {
    try {
      const response = await dispatch(selectListFileUploadApi({
        fileId: null,
        fileName: null,
        fileType: null,
        admissionPeriodId: null,
  ...
  const handleSelectListScoreAssignmentsSearch = async () => {
    try {
      const response = await dispatch(selectListFileUploadApi({
        fileId: formDataSearch.fileId,
        fileName: formDataSearch.fileName,
        fileType: formDataSearch.fileType,
        admissionPeriodId: formDataSearch.admissionPeriodId,
        userUpload: formDataSearch.userUpload,
  ...
  const editFileName = selectedFileUpdate?.name || (typeof formDataEdit.fileName === 'string' ? formDataEdit.fileName : '');
    ...
    <>
      ...
      <div className="p-4 bg-white block sm:flex items-center justify-between border-b border-gray-200 lg:mt-1.5 dark:bg-gray-800 dark:border-gray-700">
        <div className="w-full mb-1">
          <div className="mb-4">
            <h1 className="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">Danh sách tệp tin đồ án sinh viên</h1>
          </div>
          <div className="sm:flex">
            <div className="items-center hidden mb-3 sm:flex sm:divide-x sm:mb-0 dark:divide-gray-700">
              <form className="lg:pr-3">
                <div className="relative mt-1 lg:w-64 xl:w-96">
                  <label htmlFor="admission-period-id-search">Mã tệp tin đồ án sinh viên</label>
                  <input type="text" name="assignmentRegisterId" id="admission-period-id-search"
                    ...
                    placeholder="Tìm kiếm mã tệp tin đồ án" onChange={handleInputChangeSearch} />
                </div>
                <div className="relative mt-1 lg:w-64 xl:w-96">
                ...
                <tbody className="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                  {Array.isArray(listDataAssignmentRegister) && listDataAssignmentRegister.length ? (
                    listDataAssignmentRegister.map((assignmentRegister, idx) => {
                      const fileId = assignmentRegister?.fileId;
                      const fileName = assignmentRegister?.fileName;
                      const fileType = assignmentRegister?.fileType;
                      ...
                      const assignmentName = assignmentRegister?.assignmentName;
                      ...
                      return (
                        <tr key={fileId} className="hover:bg-gray-100 dark:hover:bg-gray-700">
                          <td className="w-4 p-4">
                            <div className="flex items-center">
                              <input
                                ...
                                aria-describedby="checkbox-1"
                                type="checkbox"
                                checked={selectedPeriodAssignment.has(fileId)}
                                onChange={() => handlePeriodAssignmentCheckboxChange(fileId)}
                                className="w-4 h-4 border-gray-300 rounded bg-gray-50 focus:ring-3 focus:ring-primary-300 dark:focus:ring-primary-600 dark:ring-offset-gray-800 dark:bg-gray-700 dark:border-gray-600" />
                              <label htmlFor={`checkbox-${fileId}`} className="sr-only">checkbox</label>
                            </div>
                          </td>
                          ...
                          <td className="max-w-sm p-4 overflow-hidden text-base font-normal text-gray-500 truncate xl:max-w-xs dark:text-gray-400">
                            <button
                              type="button"
                              id="download-period-assignment-button"
            ...
            <div className="relative bg-white rounded-lg shadow dark:bg-gray-800">
              ...
              <div className="flex items-start justify-between p-5 border-b rounded-t dark:border-gray-700 border-gray-200">
                <h3 className="text-xl font-semibold dark:text-white">
                  Cập nhật tệp tin đồ án sinh viên
                </h3>
                <button type="button"
                  onClick={closeEditModal}
                ...
                <form>
                  <div className="grid grid-cols-6 gap-6">
                    <div className="col-span-6 sm:col-span-3">
                      <label htmlFor="edit-admission-period-id" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Mã tệp tin đồ án sinh viên</label>
                      <input type="text" name="fileId" value={formDataEdit.fileId || ''} onChange={handleInputChangeEdit} id="edit-admission-period-id"
                        className="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                        placeholder="Mã tệp tin đồ án sinh viên" style={{ disabled: true }, { backgroundColor: '#adabab' }, { cursor: 'not-allowed' }} />
                    </div>
                    <div className="col-span-6 sm:col-span-3">
                  ...
                  <div className="grid grid-cols-6 gap-6">
                    ...
                    <div className="col-span-6 sm:col-span-3">
                      <label htmlFor="category-period-admission-edit" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Tệp tin tải liệu</label>
        ...
        <div
          ...
          <div onClick={(e) => e.stopPropagation()} className="relative w-full max-w-2xl px-4 md:h-auto">
            ...
            <div className="relative bg-white rounded-lg shadow dark:bg-gray-800">
              ...
              <div className="flex items-start justify-between p-5 border-b rounded-t dark:border-gray-700 border-gray-200">
                <h3 className="text-xl font-semibold dark:text-white">
                  Thêm mới tệp tin đồ án sinh viên
                </h3>
                <button type="button"
                  onClick={closeAddModal}
                  className="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-700 dark:hover:text-white">
              ...
              <div className="p-6 space-y-6">
                <form>
                  ...
                  <div className="grid grid-cols-6 gap-6">
                    <div className="col-span-6 sm:col-span-3">
                      <label htmlFor="file_input" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Tệp tin tải liệu</label>
          ...
          <div onClick={(e) => e.stopPropagation()} className="relative w-full max-w-md px-4 md:h-auto">
            ...
            <div className="relative bg-white rounded-lg shadow dark:bg-gray-800">
              ...
              <div className="p-6 pt-0 text-center">
                ...
                <h3 className="mt-5 mb-6 text-lg text-gray-500 dark:text-gray-400">Bạn có chắc chắn xóa tệp tin đồ án này không?</h3>
                <button
...
export default FileUploadAssignmentManagement;
