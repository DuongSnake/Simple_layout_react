...
function UserManagement() {
  ...
  //Handle case when click button edit or delete but no user selected
  const handleEnableButtonActions = () => {
    if(selectedUsers != null &&  selectedUsers.size === 0){
    //Disable edit and delete button when no user selected
      disableButtonEditDelete(true, true);
    } else if(selectedUsers != null && selectedUsers.size === 1){
    //Enable edit button and disable delete button when only 1 user selected
  ...
  const handleDeleteUser = async () => {
    try {
      const response = await dispatch(deleteApi({ listData: Array.from(selectedUsers)
       }));
      // Check if login was successful
      if (response.type.endsWith('/fulfilled')) {
        console.log("update successful:", response.payload);
