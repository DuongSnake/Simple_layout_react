...
import { createApi, updateApi, deleteApi, selectListAssignmentByPeriodTimeApi, selectListFileUploadApi } from '../../layout_admin/file_upload_assignment_management/FileUploadAssignmentManagementAPI.js';
...
const fileUploadAssignmentManagementSlice = createSlice({
  name: 'fileUploadAssignmentManagement',
  initialState,
  extraReducers: (builder) => {
    builder
      // Select List file upload handlers
      .addCase(selectListFileUploadApi.pending, (state) => {
        state.selectListFileUpload.loading = true;
        state.selectListFileUpload.error = null;
      })
      .addCase(selectListFileUploadApi.fulfilled, (state, action) => {
        state.selectListFileUpload.loading = false;
        state.selectListFileUpload.error = null;
        state.selectListFileUpload.data = (undefined === action.payload.data.data) ? null : action.payload.data.data;
        state.selectListFileUpload.totalRecord = (undefined === action.payload.data.totalRecord) ? null : action.payload.data.totalRecord;
      })
      .addCase(selectListFileUploadApi.rejected, (state, action) => {
        state.selectListFileUpload.loading = false;
        state.selectListFileUpload.error = action.payload || action.error.message;
      })
...
export default fileUploadAssignmentManagementSlice.reducer;
