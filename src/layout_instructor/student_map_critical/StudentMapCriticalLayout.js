...
function StudentMapCritical() {
  const dispatch = useDispatch();
  const listDataStudentMapInstructor = useSelector(state => state.studentMapCriticalManagement.selectListStudentByCriticalId.data);
  const totalRecord = useSelector(state => state.studentMapCriticalManagement.selectListStudentByCriticalId.totalRecord);
  const listDataStudentMapInstructorLoading = useSelector(state => state.studentMapCriticalManagement.selectListStudentByCriticalId.loading);
  ...
  const handleSelectListUsers = async (instructorId) => {
    try {
      const response = await dispatch(selectListStudentByCriticalIdApi({ 
        criticalId: instructorId,
  ...
  const handleSelectListUsersSearch = async () => {
    try {
      const response = await dispatch(selectListStudentByCriticalIdApi({ 
        criticalId: userIdGetFromAccountLogin.id,
        pageRequestDto : { pageNum: pager.pageNum, pageSize: pager.pageSize }
       }));
      if (response.type.endsWith('/fulfilled')) {
        // Redux selector listDataStudentMapInstructor will reflect the updated value on next render
...
export default StudentMapCritical;