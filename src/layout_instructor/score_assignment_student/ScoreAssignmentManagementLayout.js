...
import { selectListAssignmentByPeriodTimeApi, createApi, updateApi, deleteApi } from "../../layout_admin/score_assignment_management/ScoreAssignmentManagementAPI";
...
function ScoreAssignmentManagement() {
  ...
  const listDataAssignmentRegister = useSelector(state => state.scoreAssignmentManagement.selectListScoreAssignmentInstructorSite.data);
  const totalRecord = useSelector(state => state.scoreAssignmentManagement.selectListScoreAssignmentInstructorSite.totalRecord);
  const listDataAssignmentStudentRegisterLoading = useSelector(state => state.scoreAssignmentManagement.selectListScoreAssignmentInstructorSite.loading);
  ...
  const _onChangePagination = (page, pageSize) => {
    ...
    handleSelectListScoreAssignments(page, pageSize, userIdGetFromAccountLogin.id);
  };
  ...
  const handleSelectUserIdGetFromAccountLogin = async () => {
    ...
    handleSelectListScoreAssignments(pager.pageNum, pager.pageSize, valueInstructorId);
        } else {
  ...
  const handleCreate = async () => {
    try {
      ...
      if (response.type.endsWith('/fulfilled')) {
        ...
        setTimeout(() => {
          handleSelectListScoreAssignments(pager.pageNum, pager.pageSize, userIdGetFromAccountLogin.id); // Refresh period assignment list after creation
        }, 500);
      } else {
  ...
  const handleUpdate = async () => {
    try {
      ...
      if (response.type.endsWith('/fulfilled')) {
        ...
        setTimeout(() => {
          handleSelectListScoreAssignments(pager.pageNum, pager.pageSize, userIdGetFromAccountLogin.id); // Refresh period assignment list after update
        }, 500);
      } else {
  ...
  const handleSelectListAssignmentByPeriodTime = async (admissionPeriodId) => {
    try {
      const response = await dispatch(selectListAssignmentRegisterInstructorSiteApi({ 
        admissionPeriodId: admissionPeriodId,
        instructorId: userIdGetFromAccountLogin.id,
        typeApprove : 1
      }));
  ...
  const handleDeleteAdmissionPeriod = async () => {
    try {
      ...
      if (response.type.endsWith('/fulfilled')) {
        ...
        setTimeout(() => {
          handleSelectListScoreAssignments(pager.pageNum, pager.pageSize, userIdGetFromAccountLogin.id); // Refresh period assignment list after deletion
        }, 500);
      closeDeleteModal();
      } else {
  ...
  const handleSelectListScoreAssignments = async (pageNum, pageSize, userId) => {
    try {
      const response = await dispatch(selectListScoreAssignmentInstructorSiteApi({ 
        scoreAssignmentId: null, 
        assignmentRegisterId: null,
        fromDate: null,
        toDate: null,
